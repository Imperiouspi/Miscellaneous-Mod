...
public class TeleporterDarkeRealme extends Teleporter{
	 private final WorldServer worldServerInstance;
	    ...
	    public TeleporterDarkeRealme(WorldServer par1WorldServer)
	    {
	            ...
	            for (int l = -2; l <= 2; ++l)
	            {
	                for (int i1 = -2; i1 <= 2; ++i1)
	                {
	                    for (int j1 = -1; j1 < 3; ++j1)
	                    {
	                        ...
	                        int i2 = k + i1 * b1 - l * b0;
	                        boolean flag = j1 < 0;
	                        this.worldServerInstance.setBlock(k1, l1, i2, flag ? BlockInfo.DUSTBLOCK_ID : 0);
	                    }
	                }
	            }
	    ...
	    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
	    {
	        ...
	        else
	        {
	            for (k1 = l - short1; k1 <= l + short1; ++k1)
	            {
	                ...
	                for (int l1 = i1 - short1 ; l1 <= i1 + short1; ++l1)
	                {
	                    ...
	                    {
	                        if (this.worldServerInstance.getBlockId(k1, i2, l1) == BlockInfo.ABYSMAL_ID)
	                        {
	                            while (this.worldServerInstance.getBlockId(k1, i2 - 1, l1) == BlockInfo.ABYSMAL_ID)
	                            {
	                                --i2;
	                            }
	        ...
	        if (d3 >= 0.0D)
	        {
	            ...
	            d4 = (double)k + 0.5D;
	            int j2 = -1;
	            if (this.worldServerInstance.getBlockId(i - 1, j, k) == BlockInfo.ABYSMAL_ID)
	            {
	                j2 = 2;
	            }
	            if (this.worldServerInstance.getBlockId(i + 1, j, k) == BlockInfo.ABYSMAL_ID)
	            ...
	            if (this.worldServerInstance.getBlockId(i, j, k - 1) == BlockInfo.ABYSMAL_ID)
	            {
	                j2 = 3;
	            }
	            if (this.worldServerInstance.getBlockId(i, j, k + 1) == BlockInfo.ABYSMAL_ID)
	            ...
	            for (k2 = -1; k2 <= 1; ++k2)
	            {
	                for (i3 = 1; i3 < 3; ++i3)
	                {
	                    for (l2 = -1; l2 < 3; ++l2)
	                    {
	                        ...
	                        i4 = j2 + (i3 - 1) * l5 - k2 * k5;
	                        flag = l2 < 0;
	                        this.worldServerInstance.setBlock(k3, j3, i4, flag ? BlockInfo.DUSTBLOCK_ID : 0);
	                    }
	                }
	            }
	        }
	        for (k2 = 0; k2 < 4; ++k2)
	        {
	            for (i3 = 0; i3 < 4; ++i3)
	            {
	                for (l2 = -1; l2 < 4; ++l2)
	                {
	                    ...
	                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? BlockInfo.DUSTBLOCK_ID : BlockInfo.ABYSMAL_ID, 0, 2);
	                }
	            }
	            for (i3 = 0; i3 < 4; ++i3)
	            {
